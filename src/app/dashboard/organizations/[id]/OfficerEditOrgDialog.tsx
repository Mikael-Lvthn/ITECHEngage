...
import { updateOrganizationProfile } from "@/lib/actions/org-roles";
import { createClient } from "@/lib/supabase/client";

interface OfficerEditOrgDialogProps {
    org: {
        id: string;
        name: string;
...
export default function OfficerEditOrgDialog({ org }: OfficerEditOrgDialogProps) {
    ...
    // Newly-picked files (uploaded server-side on submit) and their previews.
    const [logoFile, setLogoFile] = useState<File | null>(null);
    const [coverFile, setCoverFile] = useState<File | null>(null);
    const [logoPreview, setLogoPreview] = useState(org.logo_url || "");
    const [coverPreview, setCoverPreview] = useState(org.cover_photo_url || "");
    ...
    function handleFileSelect(event: React.ChangeEvent<HTMLInputElement>, type: "logo" | "cover") {
    ...
    return (
        <>
            <button
                ...
                className="px-4 py-2 rounded-lg border text-sm font-medium hover:bg-muted transition-colors bg-card shadow-sm flex items-center gap-1.5"
            >
                ✏️ Edit
            </button>
                    ...
                    <div className="relative bg-white border rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto animate-scale-in">
                        ...
                        <div className="p-6 sm:p-8">
                            ...
                            <p className="text-sm text-gray-500 mb-8 pb-4 border-b">
                                Update your organization&apos;s details, images, and core statements.
                            </p>

                            <form onSubmit={handleSubmit} className="space-y-6">
                                <input type="hidden" name="organization_id" value={org.id} />
                                <input type="hidden" name="logo_url" value={org.logo_url || ""} />
                                <input type="hidden" name="cover_photo_url" value={org.cover_photo_url || ""} />
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div>
                                            ...
                                            <div className="w-full h-24 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 flex items-center justify-center overflow-hidden mb-3 relative group">
                                                {coverPreview ? (
                                                    // eslint-disable-next-line @next/next/no-img-element
                                                    <img src={coverPreview} alt="Cover" className="absolute inset-0 w-full h-full object-cover group-hover:brightness-90 transition-all" />
                                                ) : (
